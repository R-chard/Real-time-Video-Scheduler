...
# Investigates the relationship between obj speed, size, cluster, occlusion percent to KCF prediction accuracy
    ...
    for i in range(len(trackers)):
        ...
        for j in range(len(bboxes)):
            ...
            if util.ifCentreIntersect(bbox, tracked_bbox) and prevPos[trackers[i]][1] ==  obj_id:
                ...
                data_points.append([iou,[speed,confidence,occlusion_percent]])

                # Update results
                speed_to_iou.append((iou,speed_to_iou))
                speed_to_iou.append((iou,size_to_iou))
                speed_to_iou.append((iou,confidence_to_iou))
                speed_to_iou.append((iou,occlusion_percent_to_iou))
...
save_regression_model(data_points)
